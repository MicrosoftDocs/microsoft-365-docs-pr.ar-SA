---
title: الحصول على جميع نقاط الضعف
description: استرداد قائمة بكل نقاط الضعف التي تؤثر على المؤسسة
...
ms.locfileid: "63583098"
---
# <a name="list-vulnerabilities"></a>نقاط الضعف في القائمة
...
استرداد قائمة بكل نقاط الضعف.
<br>يدعم [استعلامات OData V4](https://www.odata.org/documentation/).
<br>عوامل التشغيل المعتمدة في OData:
<br>```$filter``` على:  ```id```، ```name```، ```description```، ، ```cvssV3```، ```publishedOn```، ```severity```، والخصائص ```updatedOn``` .
