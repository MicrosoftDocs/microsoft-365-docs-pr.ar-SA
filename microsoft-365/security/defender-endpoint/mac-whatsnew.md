...
- يضيف هذا الإصدار دعما macOS 12.3. بدءا من macOS 12.3، [تقوم Apple بإزالة Python 2.7](https://developer.apple.com/documentation/macos-release-notes/macos-12_3-release-notes). لن يكون هناك إصدار Python مثبت مسبقا على macOS بشكل افتراضي. **الإجراء المطلوب**: 
  - يجب على المستخدمين تحديث Microsoft Defender لنقطة النهاية لنظام التشغيل Mac إلى الإصدار 101.59.50 (أو أحدث) قبل تحديث أجهزتهم إلى macOS مونتري 12.3 (أو أحدث). يعد هذا الإصدار الأدنى 101.59.50 شرطا أساسيا للقضاء على المشكلات المتعلقة ب Python مع Microsoft Defender لنقطة النهاية for Mac على macOS مونتيري.
  - بالنسبة إلى عمليات النشر عن بعد، يجب تحديث إعدادات MDM الموجودة إلى Microsoft Defender لنقطة النهاية for Mac الإصدار 101.59.50 (أو أحدث). سيؤدي الدفع عبر MDM Microsoft Defender لنقطة النهاية أقدم لإصدار Mac إلى macOS 12.3 (أو أحدث) إلى فشل في التثبيت.
...
- لم يعد macOS 10.14 (Mojave) مدعوما
...
- [أصبح الآن التحكم في الجهاز macOS](mac-device-control-overview.md) متوفرا بشكل عام
- تمت معالجة مشكلة تعذر بدء الفحص السريع من قائمة الحالة في macOS 11 (Big Sur)
...
- تحسينات الأداء (خاصة عند استخدام YARN) & إصلاحات الأخطاء

## <a name="1012750-20121022127500"></a>101.27.50 (20.121022.12750.0)

- إصلاح لانتهاء صلاحية شهادة Apple ل macOS Catalina والإصدارات السابقة. يستعيد هذا الإصلاح وظيفة إدارة الثغرات الأمنية & المخاطر (TVM).
...
- تمت إضافة رمز تبديل سطر أوامر جديد لتعطيل ملحق الشبكة: `mdatp system-extension network-filter disable`. قد يكون هذا الأمر مفيدا لاستكشاف مشكلات الشبكات التي قد تكون مرتبطة Microsoft Defender لنقطة النهاية على Mac وإصلاحها
- تحسينات الأداء & إصلاحات الأخطاء
...
- بناء الجملة الجديد لأداة `mdatp` سطر الأوامر هو الآن الصيغة الافتراضية. لمزيد من المعلومات حول بناء الجملة الجديد، راجع [موارد Microsoft Defender لنقطة النهاية على macOS](mac-resources.md#configuring-from-the-command-line)
...
- إضافة حقول جديدة إلى إخراج `mdatp --health` التحقق من حالة الوضع الخامل ومعرف مجموعة الكشف التلقائي والاستجابة على النقط النهائية
...
- تمت إضافة إعدادات جديدة للتحكم في استبقاء العناصر في محفوظات فحص مكافحة الفيروسات. يمكنك الآن [تحديد عدد الأيام للاحتفاظ بالعناصر في محفوظات الفحص](mac-preferences.md#antivirus-scan-history-retention-in-days) [وتحديد الحد الأقصى لعدد العناصر في محفوظات الفحص](mac-preferences.md#maximum-number-of-items-in-the-antivirus-scan-history)
- إصلاحات الأخطاء
...
> [!IMPORTANT]
> نحن نعمل على بناء جملة جديد ومحسن لأداة `mdatp` سطر الأوامر. بناء الجملة الجديد هو حاليا الإعداد الافتراضي في قنوات التحديث الآجل ل Insider و Insider. نحن نشجعك على فهم نفسك باستخدام بناء الجملة الجديد هذا.
>
> سنواصل دعم بناء الجملة القديم بالتوازي مع بناء الجملة الجديد وسنوفر المزيد من الاتصال حول خطة الإهمال لبناء الجملة القديم في الأشهر القادمة.

- معالجة إنذار kernel الذي حدث في بعض الأحيان عند الوصول إلى مشاركات ملفات SMB
- تحسينات الأداء & إصلاحات الأخطاء

## <a name="1010516"></a>101.05.16

- تحسينات على منطق الفحص السريع لتقليل عدد الملفات الممسوحة ضوئيا بشكل كبير
- [دعم الإكمال التلقائي](mac-resources.md#how-to-enable-autocompletion) المضاف لأداة سطر الأوامر
- إصلاحات الأخطاء
...
- إضافة القدرة على تشغيل عمليات فحص مكافحة الفيروسات من القائمة السياقية macOS. يمكنك الآن النقر بزر الماوس الأيمن فوق ملف أو مجلد في "الباحث" وتحديد **"فحص" باستخدام Microsoft Defender لنقطة النهاية**
...
- يمكنك الآن [تعيين قناة تحديث](mac-updates.md#set-the-channel-name) Microsoft Defender لنقطة النهاية على macOS تختلف عن قناة التحديث على مستوى النظام
- أيقونة منتج جديد
...
- تحسينات حول التوافق مع Time Machine
- تمت معالجة مشكلة عدم تنظيف المنتج في بعض الأحيان لكافة الملفات ضمن `/Library/Application Support/Microsoft/Defender` أثناء إلغاء التثبيت
...
> [!CAUTION]
> لضمان الحماية الكاملة لأجهزتك macOS والتوافق مع إيقاف Apple تسليم تحديثات الأمان الأصلية macOS لإصدارات نظام التشغيل الأقدم من [الحالية - 2]، لن يتم دعم نشر MDATP for Mac والتحديثات على macOS Sierra [10.12]. سيتم تسليم تحديثات وتحسينات MDATP for Mac إلى الأجهزة التي تشغل الإصدارات Catalina [10.15]، و Mojave [10.14]، و High Sierra [10.13].
>
> إذا كان لديك MDATP for Mac تم نشره بالفعل على أجهزة Sierra [10.12]، فالرجاء الترقية إلى أحدث إصدار macOS للتخلص من مخاطر فقدان الحماية.
...
- تمت إضافة القدرة على عرض محفوظات التهديدات الكاملة في واجهة المستخدم (يمكن الوصول إليها من طريقة عرض **محفوظات الحماية** )
- تحسينات الأداء & إصلاحات الأخطاء
...
- إضافة القدرة على تكوين وظيفة الحماية من الفيروسات لتشغيلها في [الوضع السلبي](mac-preferences.md#enforcement-level-for-antivirus-engine)
- تحسينات الأداء & إصلاحات الأخطاء

## <a name="1006528"></a>100.65.28

- تمت إضافة دعم macOS Catalina
  ...
  > - بالنسبة إلى عمليات النشر المدارة، راجع الإرشادات المحدثة في [مواضيع النشر المستندة إلى JAMF](mac-install-with-jamf.md) والنشر [المستند إلى Microsoft Intune](mac-install-with-intune.md#create-system-configuration-profiles).
