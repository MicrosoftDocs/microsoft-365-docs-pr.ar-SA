...
إرجاع معلومات حول كافة أنشطة المعالجة.
...
<br>عوامل التشغيل المدعومة من OData:
<br>```$filter``` على:  ```createdon``` والخصائص ```status``` .
<br>```$top``` بقيمة قصوى 10000.
<br>```$skip```.
<br>راجع الأمثلة في [استعلامات OData مع Microsoft Defender لنقطة النهاية](exposed-apis-odata-samples.md).
...
مفوض (حساب العمل أو المؤسسة التعليمية)|RemediationTask.Read|\'قراءة معلومات الثغرة الأمنية لإدارة المخاطر والثغرات الأمنية\'
...
عنوان|سلسلة|عنوان نشاط المعالجة هذا|تحديث Microsoft Silverlight
