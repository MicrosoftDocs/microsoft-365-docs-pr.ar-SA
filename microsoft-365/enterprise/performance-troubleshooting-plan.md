...
- ابحث عن كمبيوتر عميل يمكنه إعادة إنتاج مشكلة الأداء. سيتم استخدام هذا الكمبيوتر أثناء استكشاف الأخطاء وإصلاحها.
...
- اختياري: إذا كنت تختبر Exchange Online، فقم بتشغيل أداة محلل الأداء العميل Exchange من وحدة تحكم مسؤول Office 365.
...
لم؟ من خلال مسح ذاكرة التخزين المؤقت DNS، تبدأ اختباراتك مع قائمة نظيفة. من خلال مسح ذاكرة التخزين المؤقت، تقوم بإعادة تعيين محتويات محلل DNS إلى أحدث الإدخالات. تذكر أن التدفق لا يزيل إدخالات ملف HOST. إذا كنت تستخدم إدخالات ملف HOST على نطاق واسع، يجب نسخ هذه الإدخالات إلى ملف في دليل آخر ثم إفراغ الملف HOST.
...
لقطة الشاشة هذه من الإصدار الاحترافي من HTTPWatch. يمكنك فتح التتبعات التي تم أخذها في الإصدار الأساسي على كمبيوتر باستخدام إصدار Professional وقراءته هناك. قد تتوفر معلومات إضافية من التتبع من خلال هذا الأسلوب.

## <a name="problem-steps-recorder"></a>مسجل خطوات المشكلة

يسمح لك مسجل الخطوات، أو PSR.exe، بتسجيل المشاكل أثناء حدوثها. إنها أداة مفيدة جدا وسهلة التشغيل.
...
يستخدم Netmon Microsoft Intellisense في حقل عامل تصفية العرض. Intellisense، أو إكمال التعليمات البرمجية الذكية، هو تلك الخدعة حيث تكتب في فترة ويتم عرض جميع الخيارات المتاحة في مربع تحديد منسدلة. على سبيل المثال، أنت قلق بشأن تحجيم نافذة TCP، يمكنك العثور على طريقك إلى عامل تصفية (مثل  `.protocol.tcp.window < 100`) بهذه الطريقة.
...
- قم بمسح ذاكرة التخزين المؤقت لمحلل DNS، وإذا أمكن، أغلق كافة المستعرضات باستثناء المستعرض الذي تقوم بتشغيل الاختبارات فيه. إذا لم تتمكن من القيام بذلك، على سبيل المثال، إذا كان الدعم يستخدم بعض الأدوات المستندة إلى المستعرض لرؤية سطح مكتب الكمبيوتر العميل، فكن مستعدا لتصفية التتبع.
...
- استعلام DNS القياسي واستجابة DNS outlook.office365.com مع معرفات الاستعلام المطابقة. من المهم ملاحظة إزاحة الوقت لهذا التسليم، وحيث يرسل Office 365 DNS العمومي طلب تحليل الاسم في العالم. من الناحية المثالية، محليا قدر الإمكان، بدلا من منتصف الطريق في جميع أنحاء العالم.
...
- محادثة TCP SYN وTCP SYN/ACK. تؤثر العديد من الإعدادات في هذه المحادثة على أدائك.

- ثم سلسلة من نسبة استخدام الشبكة TLS:TLS، وهو المكان الذي تجري فيه محادثات شهادة TLS ومصادقة TLS. (تذكر أن البيانات مشفرة عبر SSL/TLS.)

جميع أجزاء حركة المرور مهمة ومتصلة، ولكن أجزاء صغيرة من التتبع تحتوي على معلومات مهمة من حيث استكشاف أخطاء الأداء وإصلاحها، لذلك سنركز على تلك المناطق. بالإضافة إلى ذلك، بما أننا قمنا بما يكفي Office 365 استكشاف أخطاء الأداء وإصلاحها في Microsoft لتجميع قائمة العشرة الأوائل من المشاكل الشائعة، سنركز على هذه المشكلات وكيفية استخدام الأدوات التي لدينا لإزالتها بعد ذلك.

إذا لم تكن قد قمت بتثبيتها بالفعل، فإن المصفوفة أدناه تستخدم العديد من الأدوات حيثما أمكن ذلك. يتم توفير الارتباطات إلى نقاط التثبيت. تتضمن القائمة أدوات تتبع الشبكة الشائعة مثل [Netmon](https://www.microsoft.com/download/details.aspx?id=4865) و [Wireshark](https://www.wireshark.org/)، ولكن استخدم أي أداة تتبع تشعر بالراحة معها، والتي اعتدت فيها على تصفية نسبة استخدام الشبكة. عند الاختبار، تذكر:
...
تم العثور عليها في SYN - SYN/ACK. قد لا تستفيد الأجهزة القديمة أو القديمة من تحجيم نوافذ TCP.  بدون إعدادات تحجيم نوافذ TCP المناسبة، يملأ المخزن المؤقت الافتراضي بالإعداد 16 بت في رؤوس TCP بالمللي ثانية.  لا يمكن لنسبة استخدام الشبكة الاستمرار في الإرسال حتى يتلقى العميل إقرارا بتلقي البيانات الأصلية، مما يتسبب في حدوث تأخيرات.
...
عند إنهاء الاتصالات بواسطة أجهزة الوكيل أو جدار الحماية، لا يتم إعلام العميل، وستعني محاولة استخدام Outlook Online أن الكمبيوتر العميل سيحاول، بشكل متكرر، إجراء اتصال قبل إنشاء اتصال جديد. قد ترى توقفا في المنتج أو المطالبات أو الأداء البطيء على تحميل الصفحة.
...
لتعقب زمن الانتقال في عملية تتبع، ستستفيد من تسجيل عنوان IP لجهاز الكمبيوتر العميل وعنوان IP لخادم DNS في Office 365. هذا لتصفية التتبع بسهولة أكبر. إذا قمت بالاتصال من خلال وكيل، فستحتاج إلى عنوان IP لجهاز الكمبيوتر العميل وعنوان IP للوكيل/الخروج وعنوان IP Office 365 DNS لتسهيل العمل.
...
حيث في العالم Office 365 تحاول حل استدعاء DNS الخاص بك يؤثر على سرعة الاتصال.
