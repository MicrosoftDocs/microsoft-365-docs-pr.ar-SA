...
قم بتعيين DEP إلى علبة بريد باستخدام cmdlet Set-Mailbox. بمجرد تعيين النهج، يمكن ل Microsoft 365 تشفير علبة البريد باستخدام المفتاح المحدد في DEP.
...
## <a name="create-a-dep-for-use-with-sharepoint-online-onedrive-for-business-and-teams-files"></a>إنشاء DEP لاستخدامه مع ملفات SharePoint Online و OneDrive for Business وTeams
...
### <a name="verify-encryption-completes-for-sharepoint-online-onedrive-for-business-and-teams-files"></a>التحقق من اكتمال التشفير لملفات SharePoint Online OneDrive for Business وTeams
...
**إذا قررت عدم استخدام Customer Key لتعيين DEPs متعددة أحمال العمل بعد الآن، فستحتاج إلى تقديم تذكرة دعم باستخدام مدخل مسؤول Microsoft وتوفير التفاصيل التالية في طلبك:**

1. FQDN للمستأجر
2. جهة اتصال المستأجر لطلب الإلحاق
3. سبب عدم الإلحاق
4. قم بتضمين ملاحظة في تذكرة الخدمة تفيد بأنه يجب توجيه الطلب إلى فريق M365 Customer Key وتضمين الحدث #

يجب الاحتفاظ بأجهزة AKV لمفتاح العميل ومفاتيح التشفير مع الأذونات المناسبة للبيانات التي سيتم إعادة توزيعها باستخدام المفاتيح المدارة من Microsoft. يرجى التواصل مع m365-ck@service.microsoft.com إذا كانت لديك أي أسئلة.
...
> [!IMPORTANT]
> لا يتم دعم العودة من مفتاح العميل إلى المفاتيح المدارة من Microsoft لملفات SharePoint Online OneDrive for Business وTeams.
...
يقوم Microsoft 365 بتدقيق مسار إزالة البيانات والتحقق من صحته. لمزيد من المعلومات، راجع تقرير SSAE 18 SOC 2 المتوفر على [Service Trust Portal](https://servicetrust.microsoft.com/). بالإضافة إلى ذلك، توصي Microsoft بالمستندات التالية:
...
4. اتصل بدعم Microsoft واطلب eDocument لإزالة البيانات.
    ...
    بناء على طلبك، ترسل لك Microsoft مستندا قانونيا للإقرار بحذف البيانات وتخويله. يحتاج الشخص في مؤسستك الذي قام بالتسجيل كموافق في عرض FastTrack أثناء الإلحاق إلى توقيع هذا المستند. عادة ما يكون هذا مديرا تنفيذيا أو شخصا آخر معينا في شركتك مخولا بشكل قانوني بالتوقيع على الأوراق نيابة عن مؤسستك.
...
### <a name="revoke-your-customer-keys-and-the-availability-key-for-sharepoint-online-onedrive-for-business-and-teams-files"></a>إبطال "مفاتيح العملاء" ومفتاح التوفر لملفات SharePoint Online OneDrive for Business وTeams

إزالة ملفات DEPs في SharePoint وOneDrive للعمل أو المؤسسة التعليمية وملفات Teams غير معتمدة في مفتاح العميل. تستخدم برامج DEPs متعددة حمل العمل هذه لتشفير البيانات عبر أحمال عمل متعددة عبر جميع مستخدمي المستأجرين. سيؤدي إزالة مثل هذا DEP إلى تعذر الوصول إلى البيانات من خلال أحمال عمل متعددة. إذا قررت إنهاء خدمات Microsoft 365 تماما، يمكنك متابعة مسار حذف المستأجر لكل عملية موثقة. تعرف على كيفية [حذف مستأجر في Azure Active Directory](/azure/active-directory/enterprise-users/directory-delete-howto).
