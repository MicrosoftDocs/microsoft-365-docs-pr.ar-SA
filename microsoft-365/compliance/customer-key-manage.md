...
ms.locfileid: "64762244"
---
# <a name="manage-customer-key"></a>إدارة مفتاح العميل

بعد إعداد مفتاح العميل Office 365، ستحتاج إلى إنشاء نهج تشفير بيانات واحد أو أكثر وتعيينه (DEP). بمجرد تعيين DEPs، يمكنك إدارة مفاتيحك كما هو موضح في هذه المقالة. تعرف على المزيد حول مفتاح العميل في المواضيع ذات الصلة.
...
إذا قررت عدم استخدام Customer Key لتعيين DEPs متعددة أحمال العمل بعد الآن، فستحتاج إلى التواصل مع دعم Microsoft مع طلب "إلغاء الإلحاق" من Customer Key. اطلب من فريق الدعم تقديم طلب خدمة مقابل فريق Microsoft 365 Customer Key. تواصل مع m365-ck@service.microsoft.com إذا كانت لديك أي أسئلة.
...
لبدء مسار إزالة البيانات لملفات SharePoint Online و OneDrive for Business و Teams، أكمل الخطوات التالية:

1. إبطال الوصول Key Vault Azure. يجب أن يوافق جميع مسؤولي المخزن الرئيسي على إبطال الوصول.

   لا تحذف Key Vault Azure ل SharePoint Online. يمكن مشاركة مخازن المفاتيح بين العديد من المستأجرين عبر الإنترنت SharePoint و DEPs.

2. اتصل ب Microsoft لحذف مفتاح التوفر.

    عندما تتصل ب Microsoft لحذف مفتاح التوفر، سنرسل لك مستندا قانونيا. يحتاج الشخص الذي قام بالتسجيل كموافق في عرض FastTrack أثناء الإلحاق إلى توقيع هذا المستند. عادة ما يكون هذا مديرا تنفيذيا أو شخصا آخر معينا في شركتك مخولا بشكل قانوني بالتوقيع على الأوراق نيابة عن مؤسستك.

3. بمجرد توقيع ممثلك على المستند القانوني، قم بإرجاعه إلى Microsoft (عادة من خلال توقيع eDoc).

   بمجرد تلقي Microsoft للمستند القانوني، نقوم بتشغيل cmdlets لتشغيل عملية إزالة البيانات التي تؤدي إلى حذف تشفير مفتاح المستأجر ومفتاح الموقع وجميع مفاتيح كل مستند على حدة، مما يؤدي إلى كسر التسلسل الهرمي للمفتاح بشكل لا رجعة فيه. بمجرد اكتمال cmdlets لإزالة البيانات، تتم إزالة بياناتك.
