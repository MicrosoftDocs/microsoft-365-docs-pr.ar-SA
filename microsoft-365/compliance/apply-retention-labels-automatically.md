...
إحدى أقوى ميزات [تسميات الاستبقاء](retention.md) هي القدرة على تطبيقها تلقائيا على المحتوى الذي يطابق الشروط المحددة. في هذه الحالة، لا يحتاج الأشخاص في مؤسستك إلى تطبيق تسميات الاستبقاء. Microsoft 365 يعمل من أجلهم.
...
> [!TIP]
> استخدم الخصائص القابلة للبحث لتحديد [Teams تسجيلات الاجتماع](#microsoft-teams-meeting-recordings) [والعناصر التي تم تطبيق وصف الحساسية عليها](#identify-files-and-emails-that-have-a-sensitivity-label).
...
بالإضافة إلى ذلك، SharePoint العناصر الموجودة في مسودة أو التي لم يتم نشرها من قبل غير معتمدة لهذا السيناريو.
...
> [!IMPORTANT]
> بالنسبة إلى رسائل البريد الإلكتروني التي تقوم بتطبيقها تلقائيا من خلال تحديد المعلومات الحساسة، يتم تضمين كل علب البريد تلقائيا، والتي تتضمن علب البريد من مجموعات Microsoft 365.
> 
...
- إذا كنت تستخدم أنواع معلومات حساسة مخصصة، فلن تتمكن من تسمية العناصر الموجودة تلقائيا في SharePoint OneDrive.

- بالنسبة لرسائل البريد الإلكتروني، لا يمكنك تحديد مستلمين معينين لتضمينهم أو استبعادهم؛ يتم اعتماد إعداد **كافة المستلمين** فقط، وبالنسبة لهذا التكوين فقط، فإنه يتضمن علب بريد من مجموعات Microsoft 365. 
...
- بالنسبة SharePoint، لا يتم اعتماد الخصائص المتتبعة والخصائص المخصصة لاستعلامات KQL هذه ويجب عليك استخدام الخصائص المدارة المعرفة مسبقا فقط للمستندات. ومع ذلك، يمكنك استخدام التعيينات على مستوى المستأجر مع الخصائص المدارة المعرفة مسبقا التي يتم تمكينها كمدققات بشكل افتراضي (RefinableDate00-19 و RefinableString00-99 و RefinableInt00-49 و RefinableDecimals00-09 و RefinableDouble00-09). لمزيد من المعلومات، راجع [نظرة عامة على الخصائص المدارة والمتتبعة في SharePoint Server](/SharePoint/technical-reference/crawled-and-managed-properties-overview)، وللإرشادات، راجع [إنشاء خاصية مدارة جديدة](/sharepoint/manage-search-schema#create-a-new-managed-property).

- إذا قمت بتعيين خاصية مخصصة إلى إحدى خصائص المدقق، فانتظر 24 ساعة قبل استخدامها في استعلام KQL للحصول على تسمية استبقاء.

- على الرغم من أنه يمكن إعادة تسمية الخصائص المدارة SharePoint باستخدام الأسماء المستعارة، فلا تستخدمها في استعلامات KQL في التسميات. حدد دائما الاسم الفعلي للخاصية المدارة، على سبيل المثال، "RefinableString01".
...
يعرف الاستعلام التالي SharePoint مستندات Word أو جداول بيانات Excel عندما تحتوي هذه الملفات على **كلمة مرور** الكلمات الأساسية أو **كلمات المرور** أو **pw**:

```
(password OR passwords OR pw) AND (filetype:doc* OR filetype:xls*)
```

يحدد الاستعلام التالي Exchange أي مستند Word أو PDF يحتوي على الكلمة **nda** أو عبارة **اتفاقية عدم الكشف** عند إرفاق هذه المستندات برسالة بريد إلكتروني:
...
> [!NOTE]
> لن تعمل القدرة على الاحتفاظ بتسجيلات الاجتماعات Teams وحذفها قبل حفظ التسجيلات في OneDrive أو SharePoint. لمزيد من المعلومات، راجع [استخدام OneDrive for Business SharePoint Online أو Stream لتسجيلات الاجتماعات](/MicrosoftTeams/tmr-meeting-recording-change).

لتحديد تسجيلات الاجتماعات Microsoft Teams المخزنة في حسابات المستخدمين OneDrive أو في SharePoint، حدد ما يلي **لمحرر استعلام الكلمة الأساسية**:
...
لتحديد الملفات في SharePoint أو OneDrive ورسائل البريد الإلكتروني Exchange التي تم تطبيق [وصف حساسية](sensitivity-labels.md) معين عليها، حدد ما يلي **لمحرر استعلام الكلمة الأساسية**:

```
InformationProtectionLabelId:<GUID>
```

للعثور على GUID، استخدم [get-Label](/powershell/module/exchange/get-label) cmdlet من [Security & Compliance Center PowerShell](/powershell/exchange/scc-powershell):
...
> [!TIP]
> إذا كنت تستخدم مصنفات قابلة للتدريب Exchange، فراجع [كيفية إعادة تدريب مصنف في مستكشف المحتوى](classifier-how-to-retrain-content-explorer.md).

يجب مراعاة عند استخدام المصنفات القابلة للتدريب لتطبيق تسميات الاستبقاء تلقائيا:

- لا يمكنك تسمية العناصر SharePoint والعناصر OneDrive الأقدم من ستة أشهر تلقائيا.
...
قد تحتاج إلى استخدام هذا الخيار إذا كان مطلوبا منك التقاط جميع نسخ الملفات الموجودة في المستأجر والاحتفاظ بها والتي يتم إرسالها عبر الاتصالات من قبل المستخدمين. يمكنك استخدام هذا الخيار بالتزامن مع نهج الاستبقاء لخدمات الاتصال نفسها، Exchange Teams.
...
مرفقات السحابة المعتمدة لهذا الخيار هي ملفات مثل المستندات ومقاطع الفيديو والصور المخزنة في SharePoint OneDrive. بالنسبة Teams، يتم دعم مرفقات السحابة المشتركة في رسائل الدردشة والقنوات القياسية والخاصة. مرفقات السحابة المشتركة عبر دعوات الاجتماعات والتطبيقات الأخرى غير Teams أو Outlook غير معتمدة. يجب مشاركة مرفقات السحابة من قبل المستخدمين؛ مرفقات السحابة المرسلة عبر الروبوتات غير مدعومة.

على الرغم من أنه غير مطلوب لهذا الخيار، نوصي بالتأكد من تمكين تعيين الإصدار لمواقع SharePoint والحسابات OneDrive بحيث يمكن التقاط الإصدار المشترك بدقة. إذا لم يتم تمكين تعيين الإصدار، فسيتم الاحتفاظ بآخر إصدار متوفر. المستندات الموجودة في المسودة أو التي لم يتم نشرها على الإطلاق غير معتمدة.

عند تحديد تسمية لاستخدامها لتطبيق تسميات الاستبقاء تلقائيا لمرفقات السحابة، تأكد من أن إعداد استبقاء التسمية **الذي يبدأ فترة الاستبقاء استنادا إليه** هو **وقت تسمية العناصر**. 

عند تكوين المواقع لهذا الخيار، يمكنك تحديد:

- **SharePoint مواقع** للملفات المشتركة المخزنة في مواقع اتصالات SharePoint ومواقع الفرق غير المتصلة بمجموعات Microsoft 365 والمواقع الكلاسيكية. 
- **مجموعات Microsoft 365** للملفات المشتركة المخزنة في مواقع الفريق المتصلة بمجموعات Microsoft 365.
- **OneDrive حسابات** الملفات المشتركة المخزنة في OneDrive المستخدمين.
...
> [!NOTE]
> إذا كنت تريد أن تنتهي صلاحية مرفقات السحابة التي تم الاحتفاظ بها في نفس الوقت الذي تنتهي فيه صلاحية الرسائل التي تحتوي عليها، فكون تسمية الاستبقاء بحيث يكون لها نفس الاستبقاء ثم احذف الإجراءات والتوقيتات مثل نهج الاستبقاء Exchange Teams.
...
- مرفقات السحابة المشتركة خارج Teams Outlook غير معتمدة.

- العناصر التالية غير معتمدة كمرفقات سحابية يمكن الاحتفاظ بها:
    - SharePoint المواقع والصفحات والقوائم والنماذج والمجلدات ومجموعات المستندات وصفحات OneNote.
    - الملفات التي يشاركها المستخدمون الذين ليس لديهم حق الوصول إلى تلك الملفات.
    - الملفات التي يتم حذفها قبل إرسال مرفق السحابة. قد يحدث هذا الأمر إذا قام مستخدم بنسخ مرفق تمت مشاركته مسبقا ولصقه من رسالة أخرى، دون التأكد أولا من أن الملف لا يزال متوفرا. أو يقوم شخص ما بإعادة توجيه رسالة قديمة عند حذف الملف الآن.
