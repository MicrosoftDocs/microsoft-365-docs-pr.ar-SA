...
ms.locfileid: "66787529"
---
# <a name="get-started-with-insider-risk-management-settings"></a>بدء استخدام إعدادات إدارة المخاطر الداخلية

تنطبق إعدادات إدارة المخاطر الداخلية على جميع نهج إدارة المخاطر الداخلية، بغض النظر عن القالب الذي تختاره عند إنشاء نهج. يتم تكوين الإعدادات باستخدام عنصر تحكم **إعدادات المخاطر Insider** الموجود في أعلى جميع صفحات إدارة المخاطر الداخلية. تتحكم هذه الإعدادات في مكونات النهج للمجالات التالية:
...
- [تحليلات](#analytics)
- [إعلامات مسؤول](#admin-notifications)
...
- **مؤشرات Office**: تتضمن مؤشرات النهج لمواقع SharePoint وMicrosoft Teams ومراسلة البريد الإلكتروني.
- **مؤشرات الجهاز**: تتضمن مؤشرات النهج لنشاط مثل مشاركة الملفات عبر الشبكة أو مع الأجهزة. تتضمن المؤشرات الأنشطة التي تتضمن جميع أنواع الملفات، باستثناء نشاط الملفات القابلة للتنفيذ (.exe) ومكتبة الارتباطات الديناميكية (.dll). إذا قمت بتحديد *مؤشرات الجهاز*، تتم معالجة النشاط للأجهزة التي تحتوي على أجهزة Windows 10 الإصدار 1809 أو الإصدارات الأحدث وأجهزة macOS (Catalina 10.15 أو أحدث). بالنسبة إلى كل من أجهزة Windows وmacOS، يجب أولا إلحاق الأجهزة بمدخل التوافق. تتضمن مؤشرات الجهاز أيضا الكشف عن إشارة المستعرض لمساعدة مؤسستك على اكتشاف إشارات النقل غير المصرح به للعمل عليها للملفات غير القابلة للتنفيذ التي يتم عرضها أو نسخها أو مشاركتها أو طباعتها في Microsoft Edge وGoogle Chrome. لمزيد من المعلومات حول تكوين أجهزة Windows للتكامل مع المخاطر الداخلية، راجع [القسم التالي تمكين مؤشرات الجهاز وإلحاق أجهزة Windows](insider-risk-management-settings.md#OnboardDevices) في هذه المقالة. لمزيد من المعلومات حول تكوين أجهزة macOS للتكامل مع المخاطر الداخلية، راجع القسم التالي تمكين مؤشرات الجهاز وأجهزة macOS الملحقة في هذه المقالة. لمزيد من المعلومات حول الكشف عن إشارة المستعرض، راجع [التعرف على اكتشاف إشارة مستعرض إدارة المخاطر الداخلية وتكوينه](insider-risk-management-browser-support.md).
...
لتحديد مؤشرات نهج المخاطر الداخلية التي تم تمكينها في جميع نهج المخاطر الداخلية، انتقل إلى **مؤشرات** **إعدادات** >  المخاطر في Insider وحدد مؤشر نهج واحد أو أكثر. لا يمكن تكوين المؤشرات المحددة في صفحة إعدادات المؤشرات بشكل فردي عند إنشاء نهج مخاطر من الداخل أو تحريره في معالج النهج.
...
### <a name="enable-device-indicators-and-onboard-windows-devices"></a>تمكين مؤشرات الجهاز وإلحاق أجهزة Windows
<a name="OnboardDevices"> </a>

لتمكين الكشف عن أنشطة المخاطر على أجهزة Windows وتضمين مؤشرات النهج لهذه الأنشطة، يجب أن تفي أجهزة Windows بالمتطلبات التالية ويجب إكمال خطوات الإلحاق التالية.
...
2. يجب أن يكون حساب المستخدم المستخدم لتسجيل الدخول إلى الجهاز Windows 10 حساب Azure Active Directory (AAD) نشطا. قد يكون الجهاز Windows 10 [AAD](/azure/active-directory/devices/concept-azure-ad-join) أو AAD المختلط أو Active Directory منضما أو AAD مسجلا.
3. قم بتثبيت مستعرض Microsoft Edge على جهاز نقطة النهاية للكشف عن إجراءات نشاط تحميل السحابة. راجع[، قم بتنزيل Microsoft Edge الجديد استنادا إلى Chromium](https://support.microsoft.com/help/4501095/download-the-new-microsoft-edge-based-on-chromium).

#### <a name="step-2-onboarding-devices"></a>الخطوة 2: إلحاق الأجهزة
<a name="OnboardStep2"> </a>

يجب تمكين مراقبة الجهاز وإلحاق نقاط النهاية الخاصة بك قبل أن تتمكن من الكشف عن أنشطة إدارة المخاطر الداخلية على الجهاز. يتم اتخاذ الإجراءين في مدخل التوافق في Microsoft Purview.
...
في سيناريو النشر هذا، ستقوم بإلحاق الأجهزة التي لم يتم إلحاقها بعد، وتريد فقط الكشف عن أنشطة المخاطر الداخلية على الأجهزة Windows 10.

1. افتح [مدخل التوافق في Microsoft Purview](https://compliance.microsoft.com).
...
6. اتبع الإجراءات المناسبة في [أدوات وأساليب الإلحاق للأجهزة Windows 10](/windows/security/threat-protection/microsoft-defender-atp/configure-endpoints). ينقلك هذا الارتباط إلى صفحة منتقل إليها حيث يمكنك الوصول إلى الإجراءات Microsoft Defender لنقطة النهاية التي تتطابق مع حزمة النشر التي حددتها في الخطوة 5:
    ...
    - إلحاق أجهزة Windows باستخدام نقطة نهاية Microsoft Configuration Manager
...
يمكن إلحاق أجهزة macOS (Catalina 10.15 أو إصدار أحدث) في Microsoft 365 لدعم نهج إدارة المخاطر الداخلية باستخدام إما Intune أو JAMF Pro. لمزيد من المعلومات وإرشادات التكوين، راجع [إعداد أجهزة macOS في نظرة عامة على Microsoft 365 (معاينة).](device-onboarding-macos-overview.md)
...
على سبيل المثال، تقرر تمكين مؤشرات SharePoint في إعدادات نهج المخاطر الداخلية **وتعيين حدود مخصصة** لأحداث SharePoint عند تكوين مؤشرات لنهج *تسرب بيانات* المخاطر الداخلية الجديد. أثناء العمل في معالج نهج المخاطر الداخلية، يمكنك تكوين ثلاثة مستويات أحداث يومية مختلفة لكل مؤشر SharePoint للتأثير على درجة المخاطر للتنبيهات المقترنة بهذه الأحداث.
...
- إذا كان هناك من 10 إلى 19 حدثا في SharePoint يتم تنفيذها بعد حدث تشغيل، فإن درجة المخاطر تكون أقل بطبيعتها وتميل مستويات خطورة التنبيه إلى أن تكون عند مستوى منخفض.
- إذا كان هناك ما بين 20 و29 حدثا من أحداث SharePoint التي تحدث بعد التشغيل، تكون درجة المخاطر أعلى بطبيعتها وتميل مستويات خطورة التنبيه إلى أن تكون على مستوى متوسط.
- إذا كان هناك 30 حدثا أو أكثر من أحداث SharePoint التي تحدث بعد التشغيل، فإن درجة المخاطر تكون أعلى بطبيعتها ومستويات خطورة التنبيه تميل إلى أن تكون على مستوى عال.

خيار آخر لحدود النهج هو تعيين حدث تشغيل النهج إلى نشاط أعلى من مقدار النشاط اليومي المعتاد للمستخدمين. بدلا من تعريفها بواسطة إعدادات حد معينة، يتم تخصيص كل حد ديناميكيا للأنشطة الشاذة التي تم الكشف عنها لمستخدمي النهج داخل النطاق. إذا كان نشاط الحد للأنشطة غير الطبيعية مدعوما لمؤشر فردي، يمكنك تحديد **أن النشاط أعلى من نشاط المستخدم المعتاد لليوم** في معالج النهج لهذا المؤشر. إذا لم يكن هذا الخيار مدرجا، فلن يتوفر تشغيل النشاط الشاذ للمؤشر. إذا كان **النشاط أعلى من نشاط المستخدم المعتاد لليوم** يتم سرده لمؤشر، ولكن ليس قابلا للتحديد، فستحتاج إلى تمكين هذا الخيار في **مؤشرات** **نهج إعدادات** >  المخاطر في Insider.
...
1. تمكين دعم واجهة برمجة تطبيقات نشاط الإدارة Office 365 في **تنبيهات تصدير** **إعدادات** >  إدارة  > **المخاطر ل Insider**. بشكل افتراضي، يتم تعطيل هذا الإعداد لمؤسسة Microsoft 365.
...
على سبيل المثال، تحتاج إلى الحماية من تسرب البيانات لمشروع سري للغاية حيث يمكن للمستخدمين الوصول إلى المعلومات الحساسة. يمكنك اختيار إنشاء مجموعة *المستخدمين ذات الأولوية* لمستخدمي *المشروع السري* للمستخدمين في مؤسستك الذين يعملون على هذا المشروع. بالإضافة إلى ذلك، يجب ألا يكون لدى مجموعة المستخدمين ذات الأولوية هذه مستخدمين وتنبيهات وحالات وتقارير مقترنة بمجموعة مرئية لجميع مسؤولي إدارة المخاطر الداخلية الافتراضية والمحللين والتحريين. في **الإعدادات**، يمكنك إنشاء مجموعة المستخدمين ذات الأولوية لمستخدمي *المشروع السري* وتعيين مستخدمين كمراجعين يمكنهم عرض البيانات المتعلقة بالمجموعات. باستخدام معالج النهج *وتسريبات البيانات بواسطة قالب نهج المستخدمين ذي الأولوية* ، يمكنك إنشاء نهج جديد وتعيين مجموعة مستخدمي أولوية *مستخدمي المشروع السريين* إلى النهج. الأنشطة التي يتم فحصها بواسطة النهج لأعضاء مجموعة المستخدمين ذات الأولوية *لمستخدمي المشروع السري* أكثر حساسية للمخاطر، ومن المرجح أن يقوم هؤلاء المستخدمون بإنشاء تنبيه ولديهم تنبيهات ذات مستويات خطورة أعلى.

### <a name="create-a-priority-user-group"></a>إنشاء مجموعة مستخدمين ذات أولوية

لإنشاء مجموعة مستخدمين ذات أولوية جديدة، ستستخدم عناصر تحكم الإعداد في حل **إدارة المخاطر ل Insider** في مدخل التوافق في Microsoft Purview. لإنشاء مجموعة مستخدمين ذات أولوية، يجب أن تكون عضوا في مجموعة دور *Insider Risk Management* أو *Insider Risk Management مسؤول*.

أكمل الخطوات التالية لإنشاء مجموعة مستخدمين ذات أولوية:

1. في [مدخل التوافق في Microsoft Purview](https://compliance.microsoft.com)، انتقل إلى **إدارة المخاطر في Insider** وحدد **إعدادات مخاطر Insider**.
...
لتحديث مجموعة مستخدمين ذات أولوية موجودة، ستستخدم عناصر تحكم الإعداد في حل **إدارة المخاطر ل Insider** في مدخل التوافق في Microsoft Purview. لتحديث مجموعة مستخدمين ذات أولوية، يجب أن تكون عضوا في مجموعة أدوار *Insider Risk Management* أو *Insider Risk Management مسؤول*.
...
لحذف مجموعة مستخدمين ذات أولوية موجودة، ستستخدم عناصر تحكم الإعداد في حل **إدارة المخاطر ل Insider** في مدخل التوافق في Microsoft Purview. لحذف مجموعة مستخدمين ذات أولوية، يجب أن تكون عضوا في مجموعة أدوار *Insider Risk Management* أو *Insider Risk Management مسؤول*.
...
لتكوين الأصول المادية ذات الأولوية، ستقوم بتكوين موصل التالفة المادي واستخدام عناصر تحكم الإعداد في حل **إدارة المخاطر Insider** في مدخل التوافق في Microsoft Purview. لتكوين الأصول المادية ذات الأولوية، يجب أن تكون عضوا في مجموعة دور *Insider Risk Management* أو *Insider Risk Management مسؤول*.
...
1. اتبع خطوات التكوين لإدارة المخاطر الداخلية في [مقالة بدء استخدام إدارة المخاطر الداخلية](insider-risk-management-configure.md) . في الخطوة 3، تأكد من تكوين موصل الريش الفعلي.
    ...
    > [!IMPORTANT]
    > لكي تستخدم نهج إدارة المخاطر الداخلية بيانات الإشارات المتعلقة بالمستخدمين المغادرين والمنهيين وبيانات الأحداث من الأنظمة الأساسية للتحكم الفعلي والوصول، يجب أيضا تكوين موصل الموارد البشرية Microsoft 365. إذا قمت بتمكين موصل التالفة الفعلي دون تمكين موصل Microsoft 365 HR، فستقوم نهج إدارة المخاطر الداخلية بمعالجة الأحداث فقط لأنشطة الوصول الفعلي للمستخدمين في مؤسستك.

2. في [مدخل التوافق في Microsoft Purview](https://compliance.microsoft.com)، انتقل إلى **إدارة المخاطر في Insider** وحدد **الأصول المادية ذات الأولوية لإعدادات** >  **المخاطر ل Insider**.
3. في صفحة **الأصول المادية ذات الأولوية** ، يمكنك إما إضافة معرفات الأصول المادية التي تريد الكشف عنها يدويا إلى أحداث الأصول التي تم استيرادها بواسطة موصل التالفة المادي أو استيراد ملف .csv لكافة معرفات الأصول المادية المستوردة بواسطة موصل التالفة المادي: أ) لإضافة معرفات الأصول المادية يدويا، اختر **"إضافة الأصول المادية ذات الأولوية**"، وأدخل معرف الأصول المادية،  ثم حدد **"إضافة**". أدخل معرفات الأصول المادية الأخرى ثم حدد **"Add priority physical assets** " لحفظ جميع الأصول التي تم إدخالها.
...
لحذف أصل فعلي ذي أولوية موجود، ستستخدم عناصر تحكم الإعداد في حل إدارة المخاطر ل Insider في مدخل التوافق في Microsoft Purview. لحذف أصل فعلي ذي أولوية، يجب أن تكون عضوا في مجموعة دور Insider Risk Management أو Insider Risk Management مسؤول.
...
1. في [مدخل التوافق في Microsoft Purview](https://compliance.microsoft.com)، انتقل إلى **إدارة المخاطر في Insider** وحدد **الأصول المادية ذات الأولوية لإعدادات** >  **المخاطر ل Insider**.
...
- **إنشاء سجل لحالة المخاطر الداخلية في ServiceNow**: هذا القالب مخصص للمؤسسات التي تريد استخدام حل ServiceNow لتعقب حالات إدارة المخاطر الداخلية.  في حالة ما، يمكن لمحللين ومحققي المخاطر من الداخل إنشاء سجل للحالة في ServiceNow. يمكنك تخصيص هذا القالب لتعبئة الحقول المحددة في ServiceNow استنادا إلى متطلبات مؤسستك. تتوفر تدفقات Power Automate باستخدام هذا القالب على **لوحة معلومات الحالات**. لمزيد من المعلومات حول حقول ServiceNow المتوفرة، راجع المقالة [المرجعية ServiceNow Connector](/connectors/service-now/) .
...
لإنشاء تدفق Power Automate من قالب إدارة المخاطر الداخلية الموصى به، ستستخدم عناصر تحكم الإعدادات في حل **إدارة مخاطر Insider** في مدخل التوافق في Microsoft Purview أو خيار **إدارة تدفقات Power Automate** من عنصر التحكم **Automate** عند العمل مباشرة في **لوحات معلومات** **الحالات** أو المستخدمين.

لإنشاء تدفق Power Automate في منطقة الإعدادات، يجب أن تكون عضوا في *Insider Risk Management* أو *Insider Risk Management مسؤول* مجموعة الأدوار. لإنشاء تدفق Power Automate باستخدام خيار **إدارة تدفقات Power Automate** ، يجب أن تكون عضوا في مجموعة دور إدارة المخاطر الداخلية واحدة على الأقل.

أكمل الخطوات التالية لإنشاء تدفق Power Automate من قالب إدارة المخاطر الداخلية الموصى به:

1. في [مدخل التوافق في Microsoft Purview](https://compliance.microsoft.com)، انتقل إلى **إدارة المخاطر في Insider** وحدد تدفقات **Power Automate** **لإعدادات** >  مخاطر Insider. يمكنك أيضا الوصول من صفحات **لوحات معلومات** **الحالات** أو المستخدمين عن طريق اختيار **Automate** > **Manage Power Automate flows**.
2. في صفحة **تدفقات Power Automate** ، حدد قالبا مستحسنا من **قوالب إدارة المخاطر في Insider التي قد تعجبك** في القسم الموجود على الصفحة.
...
4. بشكل افتراضي، يتم تكوين التدفقات الموصى بها مسبقا باستخدام إدارة المخاطر الداخلية الموصى بها وحقول بيانات خدمة Microsoft 365 المطلوبة لإكمال المهمة المعينة للتدفق. إذا لزم الأمر، قم بتخصيص مكونات التدفق باستخدام عنصر تحكم **إظهار الخيارات المتقدمة** وتكوين الخصائص المتوفرة لمكون التدفق.
...
قد تكون بعض العمليات وسير العمل لمؤسستك خارج قوالب تدفق إدارة المخاطر الداخلية الموصى بها وقد تحتاج إلى إنشاء تدفقات Power Automate مخصصة لمناطق إدارة المخاطر الداخلية. تتميز تدفقات Power Automate بالمرونة وتدعم التخصيص الشامل، ولكن هناك خطوات يجب اتخاذها للتكامل مع ميزات إدارة المخاطر الداخلية.
...
بشكل افتراضي، تتوفر تدفقات Power Automate التي أنشأها مستخدم لهذا المستخدم فقط. لكي يتمكن مستخدمو إدارة المخاطر الداخلية الآخرين من الوصول إلى التدفق واستخدامه، يجب أن يشارك منشئ التدفق التدفق. لمشاركة تدفق، ستستخدم عناصر تحكم الإعدادات في **حل إدارة المخاطر ل Insider** في مدخل التوافق في Microsoft Purview أو خيار **إدارة تدفقات Power Automate** من عنصر التحكم Automate عند العمل مباشرة في صفحات **لوحة معلومات** **"الحالات**" أو "المستخدمون". بمجرد مشاركة تدفق، يمكن لكل شخص تمت مشاركته معه الوصول إلى التدفق في القائمة المنسدلة لعنصر التحكم **Automate** في **لوحات معلومات** **"Case**" و"User".

لمشاركة تدفق Power Automate في منطقة الإعدادات، يجب أن تكون عضوا في *Insider Risk Management* أو *Insider Risk Management مسؤول* مجموعة الأدوار. لمشاركة تدفق Power Automate مع خيار **إدارة تدفقات Power Automate** ، يجب أن تكون عضوا في مجموعة دور إدارة المخاطر الداخلية واحدة على الأقل.
...
لتحرير تدفق، ستستخدم عناصر تحكم الإعدادات في حل **إدارة المخاطر ل Insider** في مدخل التوافق في Microsoft Purview أو خيار **إدارة تدفقات Power Automate** من عنصر التحكم **Automate** عند العمل مباشرة في **لوحات معلومات** **الحالات** أو المستخدمين.

لتحرير تدفق Power Automate في منطقة الإعدادات، يجب أن تكون عضوا في *Insider Risk Management* أو *Insider Risk Management مسؤول* مجموعة الأدوار. لتحرير تدفق Power Automate باستخدام خيار **إدارة تدفقات Power Automate** ، يجب أن تكون عضوا في مجموعة دور إدارة المخاطر الداخلية واحدة على الأقل.
...
3. حدد **إعدادات** **علامة الحذف** >  لتغيير إعداد مكون التدفق أو **حذف علامة الحذف** >  لحذف مكون تدفق.
4. حدد **"حفظ"** ثم **"إغلاق** " لإكمال تحرير التدفق.

### <a name="delete-a-power-automate-flow"></a>حذف تدفق Power Automate

لحذف تدفق، ستستخدم عناصر تحكم الإعدادات في حل **إدارة المخاطر ل Insider** في مدخل التوافق في Microsoft Purview أو خيار **إدارة تدفقات Power Automate** من عنصر التحكم **Automate** عند العمل مباشرة في **لوحات معلومات** **الحالات** أو المستخدمين. عند حذف تدفق، تتم إزالته كخيار لجميع المستخدمين.

لحذف تدفق Power Automate في منطقة الإعدادات، يجب أن تكون عضوا في *Insider Risk Management* أو *Insider Risk Management مسؤول* مجموعة الأدوار. لحذف تدفق Power Automate باستخدام خيار **إدارة تدفقات Power Automate** ، يجب أن تكون عضوا في مجموعة دور إدارة المخاطر الداخلية واحدة على الأقل.

أكمل الخطوات التالية لحذف تدفق Power Automate:

1. في [مدخل التوافق في Microsoft Purview](https://compliance.microsoft.com)، انتقل إلى **إدارة المخاطر في Insider** وحدد تدفقات **Power Automate** **لإعدادات** >  مخاطر Insider. يمكنك أيضا الوصول من صفحات **لوحات معلومات** **الحالات** أو المستخدمين عن طريق اختيار **Automate** > **Manage Power Automate flows**.
2. في صفحة **تدفقات Power Automate** ، حدد تدفقا لحذفه وحدد **Delete** من قائمة التحكم في التدفق.
...
إن تمكين دعم Microsoft Teams للحالات أمر سريع وسهل التكوين. لتمكين Microsoft Teams لإدارة المخاطر الداخلية، أكمل الخطوات التالية:

1. في [مدخل التوافق في Microsoft Purview](https://compliance.microsoft.com)، انتقل إلى **إعدادات مخاطر Insider** لإدارة  > **المخاطر في Insider**.
2. حدد صفحة **Microsoft Teams** .
3. تمكين تكامل Microsoft Teams لإدارة المخاطر الداخلية.
4. حدد **"حفظ** " لتكوينه والخروج منه.

![إدارة المخاطر الداخلية ل Microsoft Teams.](../media/insider-risk-settings-teams.png)

### <a name="create-a-microsoft-teams-team-for-existing-cases"></a>إنشاء فريق Microsoft Teams للحالات الحالية

إذا قمت بتمكين دعم Microsoft Teams لإدارة المخاطر الداخلية بعد أن تكون لديك حالات موجودة، فستحتاج إلى إنشاء فريق لكل حالة يدويا حسب الحاجة. بعد تمكين دعم Microsoft Teams في إعدادات إدارة المخاطر الداخلية، ستنشئ الحالات الجديدة تلقائيا فريق Microsoft Teams جديدا.
...
1. في [مدخل التوافق في Microsoft Purview](https://compliance.microsoft.com)، انتقل إلى **حالات** **إدارة** >  المخاطر في Insider وحدد حالة موجودة.
...
- من السهل تكوين: للبدء في عمليات المسح الضوئي للتحليلات، يمكنك تحديد تشغيل الفحص عند مطالبتك بتوصية التحليلات أو الانتقال إلى **Insider risk Settings Analytics** >  وتمكين التحليلات.
...
- **سجلات تدقيق Microsoft 365**: مضمنة في جميع عمليات الفحص، هذا هو المصدر الأساسي لتحديد معظم الأنشطة التي يحتمل أن تكون محفوفة بالمخاطر.
...
- **موصل بيانات Microsoft 365 HR**: إذا تم تكوينه، تساعد أحداث موصل الموارد البشرية في تحديد الأنشطة المحفوفة بالمخاطر المقترنة بالمستخدمين الذين لديهم تواريخ إنهاء مؤقتة أو قادمة.
...
لتمكين تحليلات المخاطر الداخلية، يجب أن تكون عضوا في مجموعة دور مسؤول *Insider أو Insider* *Risk Management مسؤول* أو *Microsoft 365 Global admin*.
أكمل الخطوات التالية لتمكين تحليلات المخاطر الداخلية:

1. في [مدخل التوافق في Microsoft Purview](https://compliance.microsoft.com)، انتقل إلى **إدارة المخاطر في Insider**.
2. حدد **"تشغيل الفحص** " على **"المسح الضوئي" بحثا عن المخاطر الداخلية في بطاقة مؤسستك** على علامة التبويب **"نظرة عامة على** إدارة المخاطر من الداخل". يؤدي ذلك إلى تشغيل فحص التحليلات لمؤسستك. يمكنك أيضا تشغيل الفحص في مؤسستك من خلال الانتقال إلى **Insider risk Settings Analytics** >  وتمكين **فحص نشاط مستخدم المستأجر لتحديد المخاطر الداخلية المحتملة**.
...
بعد اكتمال فحص التحليلات الأول لمؤسستك، سيتلقى أعضاء مجموعة دور *مسؤول Insider Risk Management* إعلاما بالبريد الإلكتروني تلقائيا ويمكنهم عرض الرؤى الأولية والتوصيات لأنشطة يحتمل أن تكون محفوفة بالمخاطر من قبل المستخدمين. تستمر عمليات الفحص اليومية ما لم تقم بإيقاف تشغيل التحليلات لمؤسستك. يتم توفير إعلامات البريد الإلكتروني للمسؤولين لكل فئة من الفئات الثلاث داخل النطاق للتحليلات (تسرب البيانات والسرقة والتسريب) بعد المثيل الأول للنشاط في مؤسستك. لا يتم إرسال إعلامات البريد الإلكتروني إلى المسؤولين للكشف عن نشاط المتابعة الناتج عن عمليات الفحص اليومية. إذا تم تعطيل التحليلات في **Insider risk management** > **Analytics** >  ثم إعادة تمكينها في مؤسستك، يتم إعادة تعيين إعلامات البريد الإلكتروني التلقائية ويتم إرسال رسائل البريد الإلكتروني إلى أعضاء مجموعة دور *Insider Risk Management مسؤول* للحصول على رؤى فحص جديدة.
...
لإيقاف تشغيل تحليلات المخاطر الداخلية، يجب أن تكون عضوا في مجموعة دور مسؤول *Insider* أو *Insider Risk Management مسؤول* أو Microsoft 365 *Global admin*. بعد تعطيل التحليلات، ستظل تقارير التحليلات المعرفية ثابتة ولن يتم تحديثها للمخاطر الجديدة.

أكمل الخطوات التالية لإيقاف تشغيل تحليلات المخاطر الداخلية:

1. في [مدخل التوافق في Microsoft Purview](https://compliance.microsoft.com)، انتقل إلى **إدارة المخاطر في Insider**.
2. حدد صفحة **Insider risk settings** > **Analytics** .
3. في صفحة **Analytics** ، أوقف تشغيل **فحص نشاط مستخدم المستأجر لتحديد المخاطر الداخلية المحتملة**.

## <a name="admin-notifications"></a>إعلامات مسؤول

ترسل الإعلامات مسؤول إعلاما بالبريد الإلكتروني تلقائيا إلى مجموعات أدوار إدارة المخاطر الداخلية القابلة للتحديد. يمكنك تمكين الإعلامات وتعيين مجموعات الأدوار التي ستتلقى الإعلامات للسيناريوهات التالية:
...
إذا قمت بتمكين تحليلات إدارة المخاطر الداخلية لمؤسستك، فسيتلقى أعضاء مجموعة دور *Insider Risk Management مسؤول* إعلاما بالبريد الإلكتروني تلقائيا للحصول على نتائج تحليلات أولية لتسريب البيانات والسرقة وأنشطة النقل غير المصرح به.

إذا كنت تفضل تعطيل إعلامات المسؤول والتحليلات، فأكمل الخطوات التالية:

1. في [مدخل التوافق في Microsoft Purview](https://compliance.microsoft.com)، انتقل إلى **إعدادات مخاطر Insider** لإدارة  > **المخاطر في Insider**.
2. حدد صفحة **إعلامات مسؤول**.
