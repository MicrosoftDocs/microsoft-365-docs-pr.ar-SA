...
Office 365 E5، Microsoft 365 E5، التوافق في Microsoft 365 E5، Microsoft 365 E5 حماية البيانات & إدارة SKUs مفتاح العميل. خدمات الامتثال المتطورة في Office 365 لم تعد SKU متوفرة للحصول على تراخيص جديدة. سيستمر دعم خدمات الامتثال المتطورة في Office 365 التراخيص الموجودة.

لفهم المفاهيم والإجراءات في هذه المقالة، راجع وثائق [Key Vault Azure](/azure/key-vault/). تعرف أيضا على المصطلحات المستخدمة في Azure، على سبيل المثال، [مستأجر Azure AD](/previous-versions/azure/azure-services/jj573650(v=azure.100)#what-is-an-azure-ad-tenant).
...
- [إنشاء اشتراك Azure Key Vault في كل اشتراك](#create-a-premium-azure-key-vault-in-each-subscription)
...
- [ارجع إلى Azure Key Vault](#back-up-azure-key-vault)

- [التحقق من صحة إعدادات Key Vault Azure](#validate-azure-key-vault-configuration-settings)

- [الحصول على URI لكل Key Vault Azure](#obtain-the-uri-for-each-azure-key-vault-key)
...
أكمل هذه المهام في Azure Key Vault. ستحتاج إلى إكمال هذه الخطوات لجميع DEPs التي تستخدمها مع Customer Key.
  
### <a name="create-two-new-azure-subscriptions"></a>إنشاء اشتراكين جديدين في Azure

يتطلب "مفتاح العميل" اشتراكين في Azure. من أفضل الممارسات، توصي Microsoft بإنشاء اشتراكات Azure جديدة لاستخدامها مع "مفتاح العميل". لا يمكن Key Vault مفاتيح Azure إلا للتطبيقات في نفس مستأجر Azure Active Directory (Microsoft Azure Active Directory)، يجب إنشاء الاشتراكات الجديدة باستخدام مستأجر Azure AD نفسه المستخدم مع مؤسستك حيث سيتم تعيين DEPs. على سبيل المثال، استخدام حساب العمل أو المؤسسة الدراسية الذي لديه امتيازات المسؤول العام في مؤسستك. للحصول على خطوات مفصلة، راجع [التسجيل في Azure كمنظمة](/azure/active-directory/fundamentals/sign-up-organization).
...
1. باستخدام حساب العمل أو المؤسسة الدراسية الذي لديه أذونات المسؤول العام في مؤسستك، سجل الدخول إلى Microsoft FastTrack [الإلكتروني](https://fasttrack.microsoft.com/).
...
3. اتصل ب Microsoft لإكمال العملية.
   ...
   - لتمكين "مفتاح العميل" لتعيين DEPs لتشفير المحتوى عبر أحمال عمل Microsoft 365 متعددة (Exchange Online، Teams، حماية البيانات في Microsoft) لجميع مستخدمي المستأجر، اتصل [m365-ck@service.microsoft.com.](mailto:m365-ck@service.microsoft.com)
...
### <a name="create-a-premium-azure-key-vault-in-each-subscription"></a>إنشاء اشتراك Azure Key Vault في كل اشتراك

تم توثيق الخطوات اللازمة لإنشاء مخزن رئيسي في بدء استخدام [Azure Key Vault](/azure/key-vault/general/overview)، الذي يرشدك عبر تثبيت Azure PowerShell وإطلاقه، والاتصال باشتراكك في Azure، وإنشاء مجموعة موارد، وإنشاء مخزن رئيسي في مجموعة الموارد هذه.
  
عند إنشاء مخزن مفاتيح، يجب اختيار SKU: إما قياسي أو Premium. تسمح وحدة SKU القياسية لحماية مفاتيح azure Key Vault مع البرامج - لا توجد حماية لمفاتيح وحدة أمان الأجهزة (HSM) - ويسمح وحدة SKU ل Premium باستخدام HSMs لحماية مفاتيح Key Vault. يقبل "مفتاح العميل" المخزنات الرئيسية التي تستخدم SKU، على الرغم من أن Microsoft توصي بشدة باستخدام Premium SKU فقط. تكلفة العمليات باستخدام مفاتيح من أي من النوعين هي نفسها، لذا فإن الفرق الوحيد في التكلفة هو التكلفة لكل شهر لكل مفتاح محمي HSM. راجع [Key Vault الأسعار](https://azure.microsoft.com/pricing/details/key-vault/) للحصول على التفاصيل.
...
يتطلب إنشاء المخازن الرئيسية أيضا إنشاء مجموعات موارد Azure، نظرا لأن المخازن الرئيسية تحتاج إلى سعة تخزينية (على الرغم من أنها صغيرة) كما أن Key Vault التسجيل، إذا تم تمكينه، يؤدي أيضا إلى إنشاء بيانات مخزنة. من أفضل الممارسات توصي Microsoft باستخدام مسؤولي منفصلين لإدارة كل مجموعة موارد، مع الإدارة التي تتوافق مع مجموعة المسؤولين الذين سيديرون كل موارد "مفتاح العميل" ذات الصلة.
...
- **المساهمون الرئيسيون** في المخزن الذين يمكنهم تغيير الأذونات على Azure Key Vault نفسها. ستحتاج إلى تغيير هذه الأذونات عندما يغادر الموظفون فريقك أو ينضمون إليه. في الحالات النادرة التي يحتاج فيها المسؤولون الرئيسيون في المخزن إلى إذن شرعي لحذف مفتاح أو استعادته، ستحتاج أيضا إلى تغيير الأذونات. يجب منح هذه المجموعة من المساهمين الأساسيين في المخزن دور المساهم في المخزن الرئيسي. يمكنك تعيين هذا الدور باستخدام Azure Resource Manager. للحصول على خطوات مفصلة، [راجع استخدام Role-Based Access Control لإدارة الوصول إلى موارد اشتراك Azure](/azure/active-directory/role-based-access-control-configure). لدى المسؤول الذي ينشئ اشتراك هذا الوصول ضمنيا، والقدرة على تعيين مسؤولي آخرين لدور المساهم.
...
- أذونات **Microsoft 365** لكل مخزن مفاتيح تستخدمه لمفتاح العميل، ستحتاج إلى منح wrapKey، إلغاء الاختصار، والحصول على الأذونات إلى Microsoft 365 أساسي الخدمة. 
  ...
  لإعطاء الإذن Microsoft 365 أساسي للخدمة، قم بتشغيل الأمر **Set-AzKeyVaultAccessPolicy** cmdlet باستخدام بناء الجملة التالي:
   ...
   - بالنسبة إلى نهج حمل العمل المتعدد (Exchange أو Teams أو حماية البيانات في Microsoft) الذي ينطبق على جميع مستخدمي المستأجرين، *استبدل Office 365 التطبيق* ب`c066d759-24ae-40e7-a56f-027002b5d3e4`
...
هناك طريقتان لإضافة مفاتيح إلى azure Key Vault؛ يمكنك إنشاء مفتاح مباشرة في Key Vault، أو يمكنك استيراد مفتاح. إن إنشاء مفتاح مباشرة في Key Vault أقل تعقيدا، ولكن استيراد مفتاح يوفر التحكم الكامل في كيفية إنشاء المفتاح. استخدم مفاتيح RSA. لا يدعم Key Vault Azure التفاف المفاتيح وإبطالها باستخدام مفاتيح المنحنى الشكلي.
...
- تتضمن مجموعة الأدوات شهادة من nCipher بأن عالم أمان azure Key Vault تم إنشاؤه أيضا على HSM أصلي تم تصنيعه بواسطة nCipher. تثبت هذه الشهادة أن Microsoft تستخدم أيضا أجهزة nCipher الأصلية.
...
Microsoft 365 تعيين اشتراك Azure Key Vault إلى عدم الإلغاء وتمكين الحذف الناعم للمفاتيح التي يستخدمها "مفتاح العميل". يمكنك تأكيد إعدادات الاشتراكات بالنظر إلى مستوى الاسترداد على المفاتيح.
...
### <a name="back-up-azure-key-vault"></a>ارجع إلى Azure Key Vault

بعد إنشاء مفتاح أو تغييره مباشرة، قم بإجراء نسخة احتياطية من النسخة الاحتياطية وتخزينها، سواء عبر الإنترنت أو بدون اتصال. لا تتصل بنسخ غير متصلة بأي شبكة. بدلا من ذلك، قم بتخزينها في موقع غير متصل، مثل مرفق تخزين تجاري أو آمن. يجب تخزين نسخة واحدة على الأقل من النسخة الاحتياطية في موقع يمكن الوصول إليه في حالة حدوث كارثة. إن العناصر النقطية الاحتياطية هي الوسيلة الوحيدة لاستعادة المواد الأساسية Key Vault أن يتم إتلاف المفتاح بشكل دائم أو عرضه بطريقة أخرى غير قابلة للاستخدام. لا تتأهل المفاتيح الخارجية ل Azure Key Vault والمستوردة إلى Azure Key Vault كنسخة احتياطية لأن بيانات التعريف الضرورية لمفتاح العميل لاستخدام المفتاح غير موجودة مع المفتاح الخارجي. يمكن استخدام نسخة احتياطية تم Key Vault Azure فقط لعمليات الاستعادة باستخدام "مفتاح العميل". لذلك، يجب إنشاء نسخة احتياطية من Azure Key Vault بعد تحميل مفتاح أو إنشائه.
...
يتم تشفير ملف الإخراج الناتج عن أمر cmdlet هذا ولا يمكن استخدامه خارج Azure Key Vault. لا يمكن استعادة النسخة الاحتياطية إلا إلى اشتراك Azure الذي تم أخذ النسخة الاحتياطية منه.
...
### <a name="validate-azure-key-vault-configuration-settings"></a>التحقق من صحة إعدادات Key Vault Azure

إن التحقق من صحة المفاتيح قبل استخدام المفاتيح في DEP اختياري، ولكن يوصى به بشدة. إذا كنت تستخدم خطوات لإعداد المفاتيح والمخزنات غير تلك الموضحة في هذه المقالة، فتحقق من صحة موارد Azure Key Vault قبل تكوين "مفتاح العميل".
...
### <a name="obtain-the-uri-for-each-azure-key-vault-key"></a>الحصول على URI لكل Key Vault Azure
